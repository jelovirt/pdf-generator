...
//            }
  ...
  attribute_set(root, style, attribute_set, properties, uses) {
    ...
    _.forEach(this.style[style], (v, p) => {
      if(_.includes(properties, p)) {
        ET.SubElement(attrSet, xsl('attribute'), {name: p}).text = utils.value(p, v)
      }
    })
    return attrSet
  }
  ...
  generate_plugin() {
    ...
    // shell XSLT
    if(this.override_shell) {
      this.run_generation(zip, shell, `${this.plugin_name}/xsl/fo/topic2fo_shell_${this.formatter}.xsl`)
    }
