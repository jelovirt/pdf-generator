...
function generateInsert(root, header, flow, type) {
  ...
  const template = ET.SubElement(root, xsl('template'), {name: `insertBody${typeCase}${flowCase}`})
  ET.SubElement(template, xsl('param'), {name:'flow-name', as:'xs:string',select:`'${type}-body-${flow}'`})
  const staticContent = ET.SubElement(template, ('fo:static-content'), {'flow-name':'{$flow-name}'})
  const block = ET.SubElement(staticContent, ('fo:block'), {'xsl:use-attribute-sets':`__body__${type}__${flow}`})
  console.log(header)
  header.forEach((field) => {
    switch (field) {
      case 'copyright':
        break
      case 'title':
        ET.SubElement(block, xsl('apply-templates'), {select:'/', mode:'dita-ot:title-metadata'})
        break
      case 'chapter':
        ET.SubElement(block, ('fo:retrieve-marker'), {'retrieve-class-name':'current-header'})
        break
      case 'folio':
      ...
      case 'folio-with-total':
        ...
        ET.SubElement(block, ('fo:page-number-citation-last'), {'ref-id': '{$e:root-id}'})
        ET.SubElement(block, xsl('text')).text = ')'
        break
    }
  })
