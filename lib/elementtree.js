...
function CData(text) {
  const element = new Element(CData)
  if (text) {
    element.text = text;
  }
  return element;
...
ElementTree.prototype.write = function (options) {
  ...
  if (options.method === 'text') {
    ...
    let qnames, namespaces, indent, indent_string
    const x = _namespaces(
      this._root,
      options.encoding,
...
function _namespaces(elem, encoding, default_namespace) {
  ...
  function add_qname(qname) {
    if (qname[0] === '{') {
      const tmp = qname.substring(1).split('}', 2)
      const uri = tmp[0]
      const tag = tmp[1]
...
function _serialize_xml(
  ...
  let tag = elem.tag
  let text = elem.text
  const tail = elem.tail
  let items
  let i

  const newlines = indent || indent === 0
