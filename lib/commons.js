...
`
  ...
  const number_levels = _(['topic', 'topic_topic', 'topic_topic_topic', 'topic_topic_topic_topic'])
    .map((s) => {
      return _.has(conf.style, s) && _.has(conf.style[s], "title-numbering") && conf.style[s]["title-numbering"] === true
    })
    .value()
  ...
  if(_.has(conf.style.topic, "title-numbering") && conf.style.topic["title-numbering"] !== true) {
  ...
  if(_.has(conf.style.fig, 'caption-position') && conf.style.fig["caption-position"] === "before") {
  ...
  // tm
  if(_.has(conf.style.tm, 'symbol-scope') && conf.style.tm['symbol-scope'] !== 'always') {
...
function generate_custom_attr(root, conf) {
  ...
  if(conf.force_page_count) {
    ...
    ET.SubElement(page_count_attr, xsl('attribute'), {name: "force-page-count"}).text = conf.force_page_count
  }
  // font family
  const foRootAttrSet = conf.attribute_set(root, "body", "__fo__root", ["font-family", "color", "text-align"])
  ...
  spacing_attr_sets.forEach((n) => {
    conf.attribute_set(root, "body", n, _.difference(conf.properties, ["start-indent"]))
  })
  // example
