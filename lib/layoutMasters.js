...
function generate_custom_attr(root, conf) {
  ...
  if(conf.body_column_count) {
    ["region-body.odd", "region-body.even"].forEach((a) => {
      ...
      ET.SubElement(region_body_attr, xsl('attribute'), {name: "column-count"}).text = conf.body_column_count
      if(conf.column_gap) {
        ET.SubElement(region_body_attr, xsl('attribute'), {name: "column-gap"}).text = conf.column_gap
      }
    })
    ...
    if(conf.index_column_count) {
      ["region-bodyindex.odd", "region-bodyindex.even"].forEach((a) => {
        const region_body_attr = ET.SubElement(root, xsl('attribute-set'), {name: a})
        ET.SubElement(region_body_attr, xsl('attribute'), {name: "column-count"}).text = conf.index_column_count
      })
    }
  }
}
